...
#
# Copyright © 2001-2007, 2009, 2010, 2012 Red Hat, Inc.
...
class scdMainWindow(object):
    ...
    def showNtpFailureDialog(self, ntpServers):
        yesno = self.showErrorDialog(
                _("Couldn't connect to one of these time servers:\n\n%s\n\n"
                  "Either none of them are available or the firewall settings "
                  "on your computer are blocking NTP connections.\n\n"
                  "Do you want to change your configuration to work around "
                  "this issue now?") % "\n".join(ntpServers),
                gtk.BUTTONS_YES_NO)
    ...
    def apply(self):
        ...
        if ntpEnabled == False:
            ...
            if not self.dateBackend.syncHardwareClock():
                self.showDialog(gtk.MESSAGE_WARNING,
                        _("Failed to synchronize hardware clock. This may be "
                          "because there is no hardware clock accessible, "
                          "e.g. when running in a virtualized environment."))
            ...
            if len(ntpServers) == 0 and not ntpBroadcastClient:
                self.showErrorDialog(
                        _("Please specify an NTP server to use or enable "
                          "NTP broadcast."),
                        gtk.BUTTONS_OK)
                return

            if self.dateBackend.writeNtpConfig(ntpServers=ntpServers,
                    ntpBroadcastClient=ntpBroadcastClient,
                    ntpLocalTimeSource=ntpLocalTimeSource,
                    ntpIburst=ntpIburst) == None:
                self.showErrorDialog(
                        _("Couldn't write either /etc/ntp.conf or "
            ...
            def child_handler(signum, stack_frame):
                ...
                if not ntpFailDialogShown and (
                        result == "" or  int(result) > 0):
            ...
            try:
                ...
                self.showErrorDialog(_("Error while trying to fork child "
                    "process: %(strerror)s") % dict(strerror=e.strerror))
                return
            ...
            dlg.set_modal(True)
            dlg.set_transient_for(self.nb.get_toplevel())
    ...
    def custom_handler(self, glade, function_name, widget_name, str1, str2,
        ...
        for module in self.custom_handler_modules:
            if ('custom_widgets' in dir(module) and
                    function_name in dir(module)):
    ...
    def __init__(self, page=None, firstboot=False, showPages=None):
        ...
        self.allPagesWidgets = {
                "datetime": "datetime_vbox",
                "timezone": "tz_vbox"
                }
        ...
        if os.access("system-config-date.glade", os.F_OK):
            self.xml = gtk.glade.XML("system-config-date.glade",
    ...
    def help_clicked(self, args):
        ...
        if path == None:
            dlg = gtk.MessageDialog(None, 0,
                    ...
                    _("The help viewer could not be found. To be able to view "
                      "help you need to install the 'yelp' package."))
            dlg.set_transient_for(self.nb.get_toplevel())
            dlg.set_position(gtk.WIN_POS_CENTER_ON_PARENT)
            dlg.run()
            dlg.destroy()
    ...
    def showDialog(self, dlg_type, text, buttons=gtk.BUTTONS_OK):

        dlg = gtk.MessageDialog(self.win, 0, dlg_type, buttons, text)

        if dlg_type == gtk.MESSAGE_ERROR:
            dlg.set_title(_("Error"))
        elif dlg_type == gtk.MESSAGE_WARNING:
    ...
    def showErrorDialog(self, text, buttons=gtk.BUTTONS_CLOSE):
