...
#
# Copyright © 2001-2003, 2005-2007, 2009, 2012 Red Hat, Inc.
#
...
dltrans = util.DynLangTranslator('system-config-date-timezones')
...
class ZoneTabEntry(object):
    ...
    def __translate_tz(self):
        ...
        translated = dltrans.ugettext(
                self._tz.replace(" ", "_")).replace("_", " ")
    ...
    @property
    def comments_translated(self):
        return dltrans.ugettext(self.comments)
...
class ZoneTab(object):
    ...
    def findNearest(self, long, lat, longmin, latmin, longmax, latmax,
            currentEntry):
        # print ("findNearest: long:%.1f lat:%.1f longmin:%.1f longmax:%.1f "
        #        "latmin:%.1f latmax:%.1f currentEntry:%s" % (long, lat,
        #         longmin, longmax, longmax, latmax, currentEntry))
        nearestEntry = None
        if longmin <= long <= longmax and latmin <= lat <= latmax:
            min = -1
            for entry in filter(lambda x: x.tz != currentEntry.tz,
                                 self.entries):
                if not (entry.lat and entry.long and
                        latmin <= entry.lat <= latmax and
                        longmin <= entry.long <= longmax):
                    continue
                dx = entry.long - long
                dy = entry.lat - lat
                dist = (dy * dy) + (dx * dx)
    ...
    def addNoGeoZones(self):
        ...
        for offset in xrange(-14, 13):
            ...
            nogeotzs.append(tz)
        for tz in nogeotzs:
            self.entries.append(ZoneTabEntry(None, None, None, "Etc/" + tz,
                None))
