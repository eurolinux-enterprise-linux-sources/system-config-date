...
class TimezoneMap(gtk.VBox):
    ...
    def map_canvas_init(self, map, viewportWidth):
        ...
        self.viewportHeight = (
                int(float(self.viewportWidth) / float(self.mapWidth) *
                    float(self.mapHeight)))
        self.long = -180.0
        self.lat = -90.0
        ...
        def zoom_adj_value_changed_cb(adj, *rest):
            # round value
            adj.set_value(float(int(adj.get_value() / self.zoom_step + 0.5) *
                self.zoom_step))
            # update labels
            self.city_labels_update()

        self.zoomAdjustment = gtk.Adjustment(self.zoom_initial,
                self.zoom_lower, self.zoom_upper, self.zoom_step,
                self.zoom_page)
    ...
    def timezone_list_init(self, default):
        ...
        for entry in self.zonetab.getEntries():
            ...
            for depth in xrange(len(tz_root_str_split)):
                ...
                if root_joined_str_1 not in self.treeStoreRoots:
                    iter = self.treeStore.append(riter)
                    self.treeStore.set_value(iter, self.columns.TZ,
                            tz_root_str_split[depth])
                    self.treeStore.set_value(iter, self.columns.COMMENTS, None)
                    self.treeStore.set_value(iter, self.columns.ENTRY, None)
                    self.treeStore.set_value(iter, self.columns.TZSORT,
                            tzsortprefix + tz_root_str_split[depth])
            ...
            if entry.comments:
                self.treeStore.set_value(iter, self.columns.COMMENTS,
                                         entry.comments_translated)
            else:
                self.treeStore.set_value(iter, self.columns.COMMENTS, "")
            self.treeStore.set_value(iter, self.columns.ENTRY, entry)
            self.treeStore.set_value(iter, self.columns.TZSORT,
                    (tzsortprefix + tz_node_str).replace('+', '+1').replace(
                        '-', '-0'))
        ...
        self.treeStore.set_sort_column_id(self.columns.TZSORT,
                gtk.SORT_ASCENDING)
    ...
    def currentCityLabel_set(self):
        if self.currentEntry.comments and len(self.currentEntry.comments):
            labelText = _('Selected city: %(city)s (%(comments)s)') % {
                    'city': self.cityname_format(
                        self.currentEntry.translated_tz),
                    'comments': self.currentEntry.comments_translated}
        else:
            labelText = _('Selected city: %s') % self.cityname_format(
                    self.currentEntry.translated_tz)
    ...
    def city_label_init(self, what):
        ...
        setattr(self, what + "MapText", root.add(gnomecanvas.CanvasText,
            text='', x=0, y=0, anchor=gtk.ANCHOR_N, fill_color='black',
    ...
    def __init__(self, zonetab, default="America/New_York",
            map='../pixmaps/map1440.png', viewportWidth=480,
            tzActionLabel=gtk.Label(
                _("Please select the nearest city in your time zone:"))):
        ...
        self.canvas.set_tooltip_text(
                _("Use button 2 or 3 for panning and the scrollwheel to zoom "
                  "in or out."))
    ...
    def zoom_set(self, factor_percent):
        ...
        self.canvas.set_pixels_per_unit(self.zoomFactor *
                float(self.viewportWidth) / float(self.mapWidth))
        ...
        self.hadj.set_property('step_increment',
                float(self.viewportWidth) / self.zoomFactor / 2)
        self.hadj.set_property('page_increment',
                float(self.viewportWidth) / self.zoomFactor)
        self.vadj.set_property('step_increment',
                float(self.viewportHeight) / self.zoomFactor / 2)
        self.vadj.set_property('page_increment',
                float(self.viewportHeight) / self.zoomFactor)
    ...
    def move_to(self, long, lat):
        ...
        zoom = (self.zoomFactor * float(self.viewportWidth) /
                float(self.mapWidth))
        self.canvas.scroll_to(int(x * zoom - self.viewportWidth / 2),
                int(y * zoom - self.viewportHeight / 2))
    ...
    def _city_label_update(self, what):
        ...
        if entry.long != None and entry.lat != None:
            x, y = self.map2canvas(entry.lat, entry.long)
            anchor, xoffset, yoffset = self.get_caption_anchor_offsets(x, y,
                    map_global)
    ...
    def mapMoveEvent(self, event):
        if not self.motion_data:
            ...
            #print (r['x'], r['y'], r['w'], r['h'], r['x'] + r['w'] - 1,
            #       r['y'] + r['h'] - 1)
            longmin, latmin, longmax, latmax = self.get_shown_region_long_lat()
            #print long, lat
            #print longmin, latmin, longmax, latmax
            self.highlightedEntry = self.zonetab.findNearest(long, lat,
                    longmin, latmin, longmax, latmax, self.currentEntry)
            self.city_labels_update()
            if self.highlightedEntry:
    ...
    def mapEvent(self, widget, event=None):
        if (event.type == gtk.gdk.MOTION_NOTIFY or
                event == gtk.gdk.ENTER_NOTIFY):
            self.mapMoveEvent(event)
    ...
    def canvasEvent(self, widget, event):
        if event.type == gtk.gdk.LEAVE_NOTIFY or self.motion_data:
            ...
            self.highlightedMapText.hide()
            self.highlightedMapBox.hide()
        elif (event.type == gtk.gdk.MOTION_NOTIFY or
                event == gtk.gdk.ENTER_NOTIFY) and not self.motion_data:
            self.arrow.show()
            self.highlightedMapText.show()
            self.highlightedMapBox.show()
        elif self.allow_zoom and (event.type == gtk.gdk.SCROLL) and (
    ...
    def setCurrent(self, entry, skipList=0):
        ...
        if self.currentEntry and self.currentEntry.tz in self.markers:
        ...
        if self.currentEntry.long and self.currentEntry.lat:
            ...
            x, y = self.map2canvas(self.currentEntry.lat,
                    self.currentEntry.long)
