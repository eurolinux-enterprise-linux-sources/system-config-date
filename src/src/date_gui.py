...
#
# Copyright © 2002-2006, 2009-2011 Red Hat, Inc.
...
import scdate.core.servicesBackend
...
class datePage(object):
    def __init__(self, dateBackend, xml):
        ...
        self.services = scdate.core.servicesBackend.Services()
        ...
        self.ntpAdvOptionsExpander = self.xml.get_widget(
                "ntp_adv_options_expander")
        self.ntpAdvOptionsExpanderLabel = self.xml.get_widget(
                "ntp_adv_options_expander_label")
        self.ntpIburstButton = self.xml.get_widget("ntp_iburst")
        self.ntpLocalTimeSourceButton = self.xml.get_widget(
                "ntp_local_timesource_check")
        self.ntpBroadcastClientCheckButton = self.xml.get_widget(
                "ntp_broadcast_check")
        ...
        self.ntpListStore = gtk.ListStore(gobject.TYPE_STRING,
                gobject.TYPE_BOOLEAN)
        self.ntpListStore.set_sort_func(self.NTPSERVER, self.list_sort)
        self.ntpListStore.set_sort_column_id(self.NTPSERVER,
                gtk.SORT_ASCENDING)
        ...
        col = gtk.TreeViewColumn(_("Server"), self.cellrend,
                text=self.NTPSERVER)
        ...
        if scdMainWindow.dateBackend.ntpProvider is None:
            # no supported NTP service is installed, disable the checkbox
            self.ntpCheckButton.set_sensitive(False)
        else:
            # Find out if NTP service is currently running. If so, activate
            # checkbox
            ntpStatus = scdMainWindow.dateBackend.isNtpRunning()

            if ntpStatus:
                self.ntpCheckButton.set_active(True)
            elif ntpStatus is None:
                if self.services == scdate.core.servicesBackend.SysVServices:
                    text = _("The NTP daemon '%(ntp_daemon)s' or its init "
                             "script '%(service_config)s' does not seem to be "
                             "working properly, or its configuration file "
                             "'%(ntp_config)s' could not be read.  "
                             "system-config-date will exit now.")
                elif (self.services ==
                        scdate.core.servicesBackend.SystemDServices):
                    text = _("The NTP daemon '%(ntp_daemon)s' or its systemd "
                             "unit configuration does not seem to be "
                             "working properly, or its configuration file "
                             "'%(ntp_config)s' could not be read.  "
                             "system-config-date will exit now.")

                text = text % {
                        'ntp_daemon': self.dateBackend.ntpProvider.daemon,
                        'service_config': "/etc/rc.d/init.d/" +
                                        self.dateBackend.ntpProvider.service,
                        'ntp_config': self.dateBackend.ntpProvider.config_file
                        }
            ...
            ntpBroadcastClient = (
                    scdMainWindow.dateBackend.getNtpBroadcastClient())
            #print "ntpBroadcastClient = " + str(ntpBroadcastClient)
            self.ntpBroadcastClientCheckButton.set_active(ntpBroadcastClient)

            self.ntpServers, self.ntpLocalTimeSource = (
                    scdMainWindow.dateBackend.getNtpServers())
    ...
    def ntpSrvReachable(self, host):
        ...
        try:
            (pid, stdin, stdout, stderr) = gobject.spawn_async(
                    argv, flags=gobject.SPAWN_STDOUT_TO_DEV_NULL |
                        gobject.SPAWN_STDERR_TO_DEV_NULL |
                        gobject.SPAWN_DO_NOT_REAP_CHILD)
        except:
            # Starting ntpdate failed, most likely because it's missing, e.g.
            # because chrony is installed instead of ntp. Don't check if the
            # NTP server is reachable in this case.
            return True
        ...
        self.modal_dialog = gtk.MessageDialog(parent=self.mainWindow,
                ...
                message_format=_("Checking whether NTP server '%s' is "
                                 "reachable...") % host)
        ...
        self.modal_dialog = None
        md.destroy()
        if (result == gtk.RESPONSE_CANCEL or
                result == gtk.RESPONSE_DELETE_EVENT):
            #print "cancel, delete"
            self.modal_dialog = gtk.MessageDialog(parent=self.mainWindow,
                    flags=gtk.DIALOG_MODAL | gtk.DIALOG_DESTROY_WITH_PARENT,
                    type=gtk.MESSAGE_QUESTION,
                    buttons=gtk.BUTTONS_YES_NO,
                    message_format=(_("You cancelled checking whether NTP "
                                      "server '%s' is reachable. Do you still "
            ...
            self.modal_dialog = gtk.MessageDialog(parent=self.mainWindow,
                ...
                type=gtk.MESSAGE_QUESTION,
                buttons=gtk.BUTTONS_YES_NO,
                message_format=_("Host '%s' is not reachable or doesn't act "
                                 "as an NTP server. Do you still want your "
                                 "changes to take effect?") % host)
            result = self.modal_dialog.run()
            self.modal_dialog.destroy()
            self.modal_dialog = None
            if result == gtk.RESPONSE_YES:
    ...
    def ntpServersTreeViewSelectRow(self, *args):
        store, iter = (
                self.ntpServersTreeView.get_selection().get_selected_rows())
        ...
        if self.added_row_reference != None:
            ...
            self.added_row_reference = None
            self.ntpListStore.remove(
                    self.ntpListStore.get_iter(row_ref.get_path()))
    ...
    def ntpSrvAddButtonClicked(self, args):
        ...
        self.ntpServersTreeView.set_cursor(path,
                self.ntpServersTreeView.get_column(self.NTPSERVER), True)
        self.ntpSrvButtonbox.set_sensitive(False)
        self.added_row_reference = gtk.TreeRowReference(self.ntpListStore,
                self.ntpListStore.get_path(iter))
    ...
    def cell_edited(self, cell, path, new_text, *args):
        # only check reachability if ntpd is used, skip for chrony
